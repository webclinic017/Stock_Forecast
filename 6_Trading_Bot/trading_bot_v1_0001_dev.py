...
def gen_stop_loss():
    ...
    # Today High
    if len(today_high) == 0:
        today_high_df = pd.DataFrame([], columns=['symbol', 'new_high'])
    else:
        today_high_df = pd.DataFrame.from_dict(today_high, orient='index')
        today_high_df = today_high_df.reset_index()
        today_high_df.columns = ['symbol', 'new_high']
    ...
    stop_loss = stop_loss \
        .merge(inv, how='left', on='symbol') \
        .merge(today_high_df, how='left', on=['symbol'])


    # Replace high    
    stop_loss.loc[:, 'high'] = \
        np.where(stop_loss['new_high']>stop_loss['high'],
...
def master_level_1():
    ...
    while 2 > 1:
        ...
        # Bug, query quote一定要從arfg啟動嗎？還是可以從其他fuction呼叫，後者的話還
        # 需要儲存嗎
