...
def list_to_csv(li, file):
    
    import csv
    
    def li_get_depth(L): return isinstance(L, list) and max(map(depth, L))+1
    
    depth = li_get_depth(li)
    
    if depth == 1:
        li = [li]
    # li = ['Name', 'Branch', 'Year', 'CGPA'] 
